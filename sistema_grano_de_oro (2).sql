...
-- Tiempo de generación: 25-03-2023 a las 06:54:22
...
SELECT id,codigo,fecha,detalle,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_utilidad_bruta` (IN `_fechainicial` DATE, IN `_fechafinal` DATE)   BEGIN
DECLARE _utilidad_bruta DECIMAL(10,2);
DECLARE _costos_totales DECIMAL(10,2);
DECLARE _ingresos_totales DECIMAL(10,2);
	SELECT SUM(total_valor) INTO _ingresos_totales  FROM VENTAS WHERE fecha BETWEEN _fechainicial AND _fechafinal;
    SELECT SUM(costo_final) INTO _costos_totales  FROM kardex WHERE fecha BETWEEN _fechainicial AND _fechafinal AND tipo='salida';
   	SET _utilidad_bruta=_ingresos_totales-_costos_totales;
SELECT _ingresos_totales,_costos_totales,_utilidad_bruta;
END$$
